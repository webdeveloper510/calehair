...
<li> Plugin now skips over logs directory with hosting using cPanel </li>
<li> Patched error analytics reporting system </li>
...
= 2.1.2 =
* Plugin now skips over logs directory with hosting using cPanel.
* Patched error analytics reporting system.

= 2.1.1 =
* Added absolute path return to error, if there is a permission issue during backup.

= 2.1.0 =
* Improved SQL writer using buffer.
* Added progressive backup check before actual backup is performed.
* Added functionality to check for PHP MEMORY_LIMIT. Recommended MEMORY_LIMIT is now 64M and greater for larger sites
* Fixed remove functionality. Was not deleting actual backup file.

= 2.0.26 =
* Fixed extra slash when downloading a backup. This was causing strict servers to present a 404.
