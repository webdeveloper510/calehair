		...
		if (!empty($template)) {
			...
			if (isset($template_content['blocks'])) {
				...
				for ($i = 0; $i < count($template_blocks); $i++) {  
					...
					if ($template_blocks[$i]['type'] == "revslider" && class_exists('RevSlider')) { block_revslider_output($template_blocks[$i]); }
					...
					if ($template_blocks[$i]['type'] == "listing") { block_listing_output($template_blocks[$i]); }
